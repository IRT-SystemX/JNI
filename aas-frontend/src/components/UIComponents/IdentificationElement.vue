    ...
    <v-container fluid class="pa-0">
        <v-list-item>
            ...
            <v-tooltip activator="parent" open-delay="600" transition="slide-x-transition">
                ...
                <div v-if="identificationObject.id && identificationObject.idType" class="text-caption"><span class="font-weight-bold">{{ 'Identification(' + identificationObject.idType + '): ' }}</span>{{ identificationObject['id'] }}</div>
            </v-tooltip>
            <!-- idShort -->
            <template v-slot:title>
                <div v-html="identificationObject.idShort ? identificationObject.idShort : ''" class="text-primary text-subtitle-1"></div>
                <div v-if="identificationObject.idType">{{ 'Identification (' + identificationObject.idType + '):' }}</div>
            </template>
            <!-- identification id -->
            <template v-slot:subtitle>
                <div v-if="identificationObject.id" v-html="identificationObject.id ? identificationObject.id : ''"></div>
            </template>
            <!-- modelType -->
