  ...
  <v-container fluid class="pa-0">
    ...
    >
      <v-row>
        <v-col :cols="isMobile ? 12 : 4" :class="isMobile ? 'bg-card' : ''">
          ...
          >
            ...
            <v-list
              nav
              class="bg-card pa-0"
              :class="isMobile ? 'mx-3 mt-3' : 'ma-3'"
            >
              <v-list-item
                v-if="WidgetFeatureActive"
                to="/dashboard"
                @click="closeMenu()"
              >
        ...
        <v-col
          :cols="isMobile ? 12 : 8"
          :class="isMobile ? 'pt-0 mb-2 px-6' : 'pt-6'"
          class="bg-card"
        >
          ...
          <v-text-field
            ...
            v-model="registryURL"
            @keydown.native.enter="connectToRegistry()"
          >
          ...
          <v-text-field
            ...
            hide-details
            class="my-3"
            label="Submodel Registry Server URL"
          ...
          <v-text-field
            ...
            hide-details
            class="my-3"
            label="AAS-Server URL"
            v-model="aasServerURL"
            @keydown.native.enter="connectToAASServer()"
          >
            <template v-slot:append-inner>
              <v-btn
                size="small"
          ...
          <v-file-input
            v-if="aasServerURL && aasServerURL != ''"
            variant="outlined"
            density="compact"
            :multiple="false"
            v-model="aasxFile"
      ...
      <!-- IRT_SystemX_logo.png Logo -->
      <v-row v-if="isMobile">
        <v-col align="center">
          <v-img
            src="IRT_SystemX_logo.png"
            max-width="260px"
            :style="{ filter: isDark ? 'invert(1)' : 'invert(0)' }"
          >
            <template #sources>
              <source srcset="@/assets/IRT_SystemX_logo.png" />
            </template>
          </v-img>
        </v-col>
      </v-row>
...
export default defineComponent({
  ...
  methods: {
    ...
    uploadAASXFile() {
      ...
      this.postRequest(path, formData, headers, context, disableMessage).then(
        (response: any) => {
          if (response.success) {
