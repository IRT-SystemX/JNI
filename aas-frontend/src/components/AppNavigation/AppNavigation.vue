  ...
  <v-container>
    ...
    <v-app-bar class="px-3" color="appBar">
      <v-row class="mx-0" align="center">
              ...
              <v-row align="center">
                ...
                <v-col class="mr-3">
                  ...
                  <v-app-bar-title
                    ...
                    >JNI Platform</v-app-bar-title
                  >
                </v-col>

                <v-col
                  v-if="keycloakEnabled && $keycloak.authenticated"
                  class="mr-3"
                >
                  <!-- App Bar Title -->
                  <v-row align="center" justify="center">
                    <v-card-actions>
                      ...
                      <v-btn variant="plain">{{ $userName }}</v-btn>
                      <v-btn
                        size="x-small"
                        @click="logout()"
                        color="primary"
        ...
        <v-img
          v-if="!isMobile"
          src="K0604_Logo_JNI_RVB_800px.png"
          max-width="250px"
          :style="{ filter: isDark ? 'invert(1)' : 'invert(0)' }"
        >
          <template #sources>
            <source srcset="@/assets/K0604_Logo_JNI_RVB_800px.png" />
          </template>
        </v-img>
        <!-- Menu Toggle (Mobile) -->
    ...
    >
      ...
      >
        ...
        <v-divider></v-divider>
        <v-card-text
          style="max-height: 200px; overflow-y: auto; max-width: 590px"
        >
          <div class="text-subtitleText text-caption">
            {{ Snackbar.extendedError }}
          </div>
        </v-card-text>
      </v-card>
    ...
    <v-footer app class="bg-appBar text-center d-flex flex-column py-0">
      <v-list-item class="px-1">
        <v-list-item-title>
          <div>
            {{ new Date().getFullYear() }} — <strong>IRT SystemX ©</strong> -
            <span style="font-size: 12px">MIT License</span>
          </div>
        </v-list-item-title>
      </v-list-item>
    </v-footer>
      ...
      <div class="mr-1 mb-6">
        ...
        >
          ...
          <v-col cols="auto" class="py-1">
            <v-btn
              :icon="menuEntry.icon"
              @click="openWindow(menuEntry.route)"
              :active="menuEntry.route == $route.path"
              style="z-index: 9990"
              size="small"
            ></v-btn>
          </v-col>
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      submodelRegistryURL: "", // Variable to store the Submodel Registry Server URL
      ...
      menuEntries: [
        ...
        },
        {
          ...
          route: "/submodelelementview",
          id: 3,
        },
        {
          name: "Visualization",
          icon: "mdi-chart-timeline-variant-shimmer",
          route: "/componentvisualization",
          id: 4,
        },
      ],
  ...
  mounted() {
    ...
    // auto connect to submodel registry server that was saved in local storage
    let SubmodelRegistryURL = window.localStorage.getItem(
      "submodelRegistryURL"
    );
  ...
  computed: {
    ...
    // Get the Env Variable for the Submodel Registry Server URL from the store
    EnvSubmodelRegistryPath() {
      return this.store.getters.getEnvSubmodelRegistryPath;
    },
  ...
  methods: {
    ...
    connectToSubmodelRegistry() {
      ...
      if (this.submodelRegistryURL != "") {
        ...
        let context = "connecting to Submodel Registry Server";
        let disableMessage = false;
        this.getRequest(path, context, disableMessage).then((response: any) => {
          if (response.success) {
            this.store.dispatch(
              "dispatchSubmodelRegistryURL",
              this.submodelRegistryURL
            ); // save the URL in the store
            window.localStorage.setItem(
              "submodelRegistryURL",
              this.submodelRegistryURL
            ); // save the URL in the local storage
          } else {
            this.store.dispatch("dispatchSubmodelRegistryURL", ""); // clear the URL in the store
    ...
    openDashboard() {
      if (this.$route.name != "Dashboard" && this.WidgetFeatureActive)
