  ...
  <v-list-item class="pt-0">
    <v-list-item-title>
      <v-text-field
        ...
        :color="
          dateTimeStampValue.value == newDateTimeStampValue ? '' : 'warning'
        "
        persistent-hint
        :hint="
          dateTimeStampValue.value == newDateTimeStampValue
            ? ''
            : 'Current Value not yet saved.'
        "
      >
        <!-- Update Value Button -->
        <template v-slot:append-inner>
          <span class="text-subtitleText">{{
            unitSuffix(dateTimeStampValue)
          }}</span>
          <v-btn
            size="small"
            variant="elevated"
    ...
    <v-row class="mt-0">
      ...
      <v-col cols="auto">
        <v-date-picker
          color="primary"
          @update:modelValue="applyDate"
          @click:cancel="revertDate()"
          elevation="1"
        ></v-date-picker>
      </v-col>
      <!-- Time Picker -->
...
export default defineComponent({
  ...
  methods: {
    ...
    updateValue() {
      // console.log("Update Value: " + this.newDateTimeStampValue);
      let path =
        this.SelectedNode.basePath +
        "/" +
        this.dateTimeStampValue.path +
        "/$value";
      let content = '"' + this.newDateTimeStampValue + '"';
      let headers = { "Content-Type": "application/json" };
      let context =
        "updating " +
        this.dateTimeStampValue.modelType +
        ' "' +
        this.dateTimeStampValue.idShort +
        '"';
      let disableMessage = false;
      // Send Request to update the value of the property
      this.patchRequest(path, content, headers, context, disableMessage).then(
        (response: any) => {
          if (response.success) {
            // this.newPropertyValue = ''; // reset input
            let updatedDateTimeStampValue = { ...this.dateTimeStampValue }; // copy the stringValue
            updatedDateTimeStampValue.value = content
              .toString()
              .replace(/"/g, ""); // update the value of the stringValue
    ...
    unitSuffix(prop: any) {
      if (
        ...
        prop.embeddedDataSpecifications.length > 0 &&
        prop.embeddedDataSpecifications[0].dataSpecificationContent &&
        prop.embeddedDataSpecifications[0].dataSpecificationContent.unit
      ) {
    ...
    applyTime() {
      ...
      this.newDateTimeStampValue =
        tempDateTimeStampValue + "T" + this.time + "." + tempStampEnd;
    },
  },
});
</script>
