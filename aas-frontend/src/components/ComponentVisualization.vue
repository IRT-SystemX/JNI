<template>
  <v-container fluid class="pa-0">
    <v-card color="rgba(0,0,0,0)" elevation="0">
      <v-card-title style="padding: 15px 16px 16px">
        <v-row justify="space-between" align="center">
          <v-col>
            <div>
              Visualization
              <span class="text-caption" v-if="!autoSyncStatus">
                - Digital Shadow Off
              </span>
              <i
                v-if="autoSyncStatus"
                class="fa fa-sm fa-circle text-red-glow blinkAnimation"
                @mouseover="showPopup = true"
                @mouseleave="showPopup = false"
              ></i>
              <!-- Popup  -->
              <div v-if="showPopup" class="popup" @click="togglePopup">
                <span class="text-caption">Digital Shadow Activated</span>
              </div>
            </div>
          </v-col>
          <v-spacer></v-spacer>
      ...
      <v-card-text
        v-if="
          submodelElementData && Object.keys(submodelElementData).length > 0
        "
        style="overflow-y: auto; height: calc(100vh - 170px)"
      >
        <v-img src="IRT_SystemX_logo.png" max-width="260px">
          <template #sources>
            <source srcset="@/assets/transfo.jpg" />
          </template>
        </v-img>
        ...
        <WidgetEntrypoint
          ...
          :submodelElementData="submodelElementData"
          :selectedNode="SelectedNodeToTransfer"
        ></WidgetEntrypoint>
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      showPopup: false, //popup of the fa- image
      autoSyncStatus: false, // auto sync status value
    };
  },
  ...
  watch: {
    // Resets the autoSyncStatus when the autoSync Status changes in the ComponentVisualisation.vue
    autoSync() {
  ...
  computed: {
    // function that retrieves the status of the auto sync vue
    autoSync() {
      return this.store.getters.getAutoSync
        ? this.store.getters.getAutoSync.state
        : false;
    },
