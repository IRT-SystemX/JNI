            ...
            <template v-slot:append="{isActive}">
                ...
                <v-tooltip v-if="isActive" text="Copy Path to Clipboard" :open-delay="600" location="bottom">
                    <template v-slot:activator="{ props }">
                        <v-icon color="subtitleText" v-bind="props" @click.stop.prevent="copyPathToClipboard(item.basePath, item.path)">{{ copyIcon }}</v-icon>
                    </template>
                </v-tooltip>
            </template>
        </v-list-item>
...
export default defineComponent({
    ...
    methods: {
        ...
        toggleNode() {
            ...
            if(localItem.isActive) {
                if (this.isMobile) {
                    ...
                    this.$router.push({ path: '/submodelelementview', query: { aas: this.SelectedAAS.endpoints[0].protocolInformation.href, aasidbase64: this.SelectedAAS.aasIdBase64, basePath: localItem.basePath, path: localItem.path } });
                } else {
                    // just add the path to the URL
                    this.$router.push({ query: { aas: this.SelectedAAS.endpoints[0].protocolInformation.href, aasidbase64: this.SelectedAAS.aasIdBase64, basePath: localItem.basePath, path: localItem.path } });
                }
            } else {
        ...
        copyPathToClipboard(basePath: string, path: string) {
            // console.log('Copy Path to Clipboard: ', basePath + '/' +  path);
            // set the icon to checkmark
            this.copyIcon = 'mdi-check';
            // copy the path to the clipboard
            navigator.clipboard.writeText(basePath + '/' +  path);
