        ...
        <v-card color="rgba(0,0,0,0)" elevation="0">
            ...
            <v-card-title class="pl-1 pr-3">
                <v-row align="center">
                    ...
                    <v-col class="pl-1 pr-0" v-if="showExtended">
                        <v-text-field variant="outlined" density="compact" hide-details label="Search for AAS..." clearable
                            @update:modelValue="filterAASList"></v-text-field>
                    </v-col>
                    <!-- Add existing AAS -->
            ...
            <v-card-text class="pa-0" :class="isMobile ? 'mobileList' : 'desktopList'">
                ...
                <v-list nav class="bg-card">
                    <!-- Single AAS -->
                    <v-list-item v-for="(AAS, i) in AASData" :key="AAS['id']" @click="selectAAS(AAS)" class="bg-listItem"
                        :class="i == AASData.length - 1 ? 'mb-0' : 'mb-2'"
                        style="border-top: solid; border-right: solid; border-bottom: solid; border-width: 1px"
                        :style="{ 'border-color': isSelected(AAS) ? primaryColor + ' !important' : (isDark ? '#686868 !important' : '#ABABAB !important') }">
                        <!-- Tooltip with idShort and identification id -->
                        <v-tooltip activator="parent" open-delay="600" transition="slide-x-transition">
                            <div class="text-caption"><span class="font-weight-bold">{{ 'idShort: ' }}</span>{{
                                AAS['idShort'] }}</div>
                            <div class="text-caption"><span class="font-weight-bold">{{ 'Identification ID: ' }}</span>{{
                                AAS['id'] }}</div>
                        </v-tooltip>
                        <!-- Icon of the AAS -->
                        ...
                        <template v-if="!drawerState" v-slot:append>
                            ...
                            <v-badge :model-value="AAS['status'] && AAS['status'] == 'offline'"
                                icon="mdi-network-strength-4-alert" color="error" text-color="buttonText" inline></v-badge>
                            <!-- Information Button -->
                            <v-btn @click.stop="showAASDetails(AAS)" icon="mdi-information-outline" size="x-small"
                                variant="plain" style="z-index: 9000"></v-btn>
                            <!-- Remove from Registry Button -->
                            <v-btn @click.stop="removeFromRegistry(AAS)" icon="mdi-close" size="x-small" variant="plain"
                                style="z-index: 9000; margin-left: -6px"></v-btn>
                        </template>
                        <v-overlay :model-value="isSelected(AAS)" scrim="primary" style="opacity: 0.2" contained
                ...
                <AASListDetails :detailsObject="detailsObject" :showDetailsCard="showDetailsCard"
                    @close-details="showDetailsCard = false" :show-extended="showExtended" />
            </v-card-text>
    ...
    methods: {
        ...
        getAASData() {
            ...
            this.getRequest(path, context, disableMessage).then((response: any) => {
                if (response.success) { // execute if the Registry Server is found
                    ...
                    let sortedData = response.data.result.sort((a: { [x: string]: { [x: string]: number; }; }, b: { [x: string]: { [x: string]: number; }; }) => (a['id'] > b['id']) ? 1 : -1);
        ...
        checkAASStatus() {
            ...
            this.AASData.forEach((AAS: any) => {
                let path = AAS.endpoints[0].protocolInformation.href + '/shells';
        ...
        selectAAS(AAS: any) {
            ...
            if (this.isMobile) {
                ...
                this.$router.push({ path: '/aastreeview', query: { aas: AAS.endpoints[0].protocolInformation.href, aasidbase64: AAS.aasIdBase64 } });
            } else {
                // Add AAS Endpoint as Query to the Router
                AAS.aasIdBase64 = btoa(AAS.id);
                this.$router.push({ query: { aas: AAS.endpoints[0].protocolInformation.href, aasidbase64: AAS.aasIdBase64 } });
            }
        ...
        isSelected(AAS: any) {
            ...
            return this.selectedAAS['endpoints'][0]['address'] === AAS['endpoints'][0]['protocolInformation']['href'];
        },
        ...
        removeFromRegistry(AAS: any) {
            ...
            if (confirm('Are you sure you want to delete the AAS from the Registry?')) {
                ...
                let path = this.registryURL + '/shell-descriptors/' + btoa(AAS.id) + '?includeAasEnvironment=true';
                ...
                this.deleteRequest(path, context, disableMessage).then((response: any) => {
                    if (response.success) { // execute if deletion was successful
                        this.store.dispatch("getSnackbar", {
                            ...
                            color: "success",
                            btnColor: "buttonText",
                            text: "Asset Administration Shell deleted successfully.",
                        }); // Show Success Snackbar
                        this.reloadList(); // reload the AAS List
                        this.$router.push({ path: '/' }); // reload path
                    }
                });
            }
        },
