...
export default defineComponent({
    ...
    watch: {
        ...
        // Resets the Treeview when the Submodel Registry Server changes
    ...
    computed: {
        ...
        // get Submodel Registry Server URL from Store
        submodelRegistryServerURL() {
            return this.store.getters.getSubmodelRegistryURL;
        },
    ...
    methods: {
        ...
        initializeTree() {
            ...
            if(!this.SelectedAAS || !this.SelectedAAS.endpoints || this.SelectedAAS.endpoints.length === 0 || !this.SelectedAAS.endpoints[0].protocolInformation.href) {
            ...
            // request to get selectedAAS submodels_refs
            let path = this.SelectedAAS.endpoints[0].protocolInformation.href + '/shells/' + this.SelectedAAS.aasIdBase64 + '/submodel-refs';
            let context = 'retrieving AAS Submodels Refs Data';
            let disableMessage = false;
            this.getRequest(path, context, disableMessage).then(async (response: any) => {
                if (response.success) {
                    let aasSumodelsRefs = this.prepareSubmodelRefsData(response.data.result);
                    // get submodels of selectedAAS from submodels-ref
                    let aasSubmodels = await this.getAASSubmodelsData(aasSumodelsRefs);
                    ...
                    let submodelData = this.prepareTreeviewData(aasSubmodels);
                    ...
                    this.store.dispatch('dispatchLoadingState', false);
                    this.store.dispatch('getSnackbar', { status: true, timeout: 60000, color: 'error', btnColor: 'buttonText', text: 'Error while retrieving AAS Submodels' }); // Show Error Snackbar
                    this.submodelData = [];
                }
            });
        },

        // Function to get the AAS submodels data
        async getAASSubmodelsData(aasSumodelsRefs: any[]) {
            let aasSubmodels:any[] = [];
            let submodelBasePath = '';

            await Promise.all(aasSumodelsRefs.map(async (submodelRef: any) => {
                //check if submodel registry url exists
                if(this.submodelRegistryServerURL != '') {
                    //get submodel server path from submodel registry server
                    let path = this.submodelRegistryServerURL + '/submodel-descriptors/' + btoa(submodelRef);
                    let submodelDescriptorResponse = await this.getRequest(path, 'retrieving Submodel Descriptor', false);
                    if (submodelDescriptorResponse.success && submodelDescriptorResponse.data && submodelDescriptorResponse.data.endpoints && submodelDescriptorResponse.data.endpoints.length > 0) {
                        submodelBasePath = submodelDescriptorResponse.data.endpoints[0].protocolInformation.href;
                    } else {
                        console.log('error cannot find submodel server path', submodelDescriptorResponse);
                    }
                } else {
                    //get submodel server path from registry server
                    submodelBasePath = this.SelectedAAS.endpoints[0].protocolInformation.href;
                }

                if (submodelBasePath === '') {
                    //error and return to prevent calls
                    return;
                }

                let path = submodelBasePath + '/submodels/' + btoa(submodelRef);
                let response = await this.getRequest(path, 'retrieving AAS Submodels Data', false);
                if (response.success) {
                    //add submodel server base path to response
                    response.data.basePath = submodelBasePath;
                    aasSubmodels.push(response.data);
                }
            }));
            return aasSubmodels;
        },

        // Function to iterate submodel-refs and prepare refs values
        prepareSubmodelRefsData(data: any[]) {
            let AASSumodelsRefs:any[] = [];
            data.forEach((element: any) => {
                if(element.keys){
                    element.keys.forEach((key: any) => {
                        AASSumodelsRefs.push(key.value);
                    });
                }
            });
            return AASSumodelsRefs;
        },
        ...
        prepareTreeviewData(data: any[]) {
            ...
            data.forEach((element: any) => {
                console.log('element base path',element.basePath)
                //copy id submodel
                element.idSubmodel = element.id; 
                //add id submodel base64
                element.idSubmodelBase64 = btoa(element.id); 
                ...
                if(element.modelType == 'Submodel') {
                    element.path = 'submodels/' + element.idSubmodelBase64;
                } else if(element.parent.modelType == 'Submodel') {
                    element.path = element.parent.path + '/submodel-elements/' + element.idShort;
                    element.basePath = element.parent.basePath;
                } else {
                ...
                } else if(element.value && Array.isArray(element.value) && element.value.length > 0 && element.modelType == 'SubmodelElementCollection') { // check for Values
